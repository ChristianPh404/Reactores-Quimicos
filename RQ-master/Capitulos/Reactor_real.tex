...
\begin{itemize}
    ...
    \item \textbf{Modelo de flujo segregado:}
...
\begin{alignat*}{3}
     & \text{Ley de velocidad:}                    &  & \frac{dC_A}{dt} = -k \cdot C_A                                      &       &                                                                  \\[10pt]
     & \text{Separación de variables:\hspace{2em}} &  & \frac{dC_A}{C_A} = -k \cdot dt                                      &       &                                                                  \\[10pt]
     & \text{Integración:}                         &  & \int_{C_{A0}}^{C_{AS}} \frac{dC_A}{C_A} = -k \int_{0}^{t} dt        &       &                                                                  \\[10pt]
     & \text{Resolución:}                          &  & \ln\left(\frac{C_{AS}}{C_{A0}}\right) = -k \cdot t                  &       &                                                                  \\[10pt]
     & \text{Forma exponencial:}                   &  & C_{AS} = C_{A0} \cdot e^{-k \cdot t}                                &       &                                                                  \\[10pt]
     & \text{Cambio a } \theta:                    &  & \mathbf{C_A(\theta) = C_{A0} \cdot \exp(-k \cdot t_R \cdot \theta)} & \quad & \textcolor{gray}{\leftarrow \text{usando } t = t_R \cdot \theta} \\[10pt]
\end{alignat*}
...
\begin{alignat*}{3}
     & \text{Relación de conversión:}              &  & C_A = C_{A0}(1 - X_A)                                              & \quad &                                                                        \\[10pt]
     & \text{Derivada de la ley:}                  &  & \cancel{C_{A0}} \frac{dX_A}{dt} = k \cdot \cancel{C_{A0}}(1 - X_A) & \quad & \textcolor{gray}{\leftarrow \frac{dC_A}{dt} = -C_{A0} \frac{dX_A}{dt}} \\[10pt]
     & \text{Separación de variables:\hspace{2em}} &  & \frac{dX_A}{1 - X_A} = k \cdot dt                                  & \quad &                                                                        \\[10pt]
     & \text{Integración:}                         &  & \int \frac{dX_A}{1 - X_A} = \int k \cdot dt                        & \quad &                                                                        \\[10pt]
     & \text{Resolución (Logaritmos):}             &  & -\ln(1 - X_A) = k \cdot t                                          & \quad &                                                                        \\[10pt]
     & \text{Forma exponencial:}                   &  & (1 - X_A) = \exp(-k \cdot t)                                       & \quad &                                                                        \\[10pt]
     & \text{Cambio a } \theta:                    &  & \mathbf{1 - X_A(\theta) = \exp(-k \cdot t_R \cdot \theta)}         & \quad & \textcolor{gray}{\leftarrow \text{usando } t = t_R \cdot \theta}       \\[10pt]
\end{alignat*}
...
\end{equation}
\noindent tal y como se explicaba en la ecuacíon %%! Terminar mañana 
\newpage
