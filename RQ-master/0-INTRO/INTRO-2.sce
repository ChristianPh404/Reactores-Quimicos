...
// Sistema de ecuaciones diferenciales, busca la evolucion en el tiempo de las variables
function dxdt = f(t,x) //el nombre de f lo decides tu 
  dxdt(1) = -c(1)*x(1)
...
c = [1 2];  // Constantes
xini = [1;0;0];  // Condiciones iniciales (valores a tiempo cero), vector columna
...
//el (:,1) todos los elementos de la primera fila para la primera columna 
for i = 1:length(t)-1
...
x = ode(xini,0,t,f);// el 0 es valor inicial t filas f columas 
plot(t,x,'-'); // Gráfica 
